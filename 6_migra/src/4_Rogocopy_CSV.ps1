...
$LogsDir = Join-Path $ScriptDir ("logs_{1}_{0}" -f $Mode, $RunStamp)
New-Item -ItemType Directory -Path $LogsDir -Force | Out-Null

$ListFile = Join-Path $ScriptDir "robocopy_jobs.csv"
$GlobalLog = Join-Path $LogsDir "_yldine_log.txt"

# robocopy src dst /E /COPY:DAT DCOPY:DAT /Z /R:2 /W:2 /NP /XJ
$RobocopyParams = @(
